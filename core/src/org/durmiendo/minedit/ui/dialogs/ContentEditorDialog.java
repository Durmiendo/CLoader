...
public class ContentEditorDialog extends BaseDialog {
    ...
    public void clazz(Class<?> clazz, Object obj) {
        if (clazz.isAssignableFrom(Content.class)) {
            cont(clazz.getName(), clazz.getSimpleName(), clazz.getPackage().getName());
            return;
        }
        row();
    ...
    public void viewFields(Table t, Class<?> c, Object obj, String s) {
        for (Field fi : R.getFields(c)) {
            ...
            if (obj == null) {
                BaseDialog bd = new BaseDialog("Класс является null");
                bd.cont.button("Инициализировать", () -> {
                    classInit(bd, fi);
                });
                bd.addCloseButton();
                hide();
            ...
            if (R.getType(fi).isEnum()) {
                enumField(t, fi, obj);
                Log.info("enum " + R.getName(fi));
            } else if (R.getType(fi).isPrimitive() || R.getType(fi).isAssignableFrom(String.class)) {
    ...
    private void classInit(BaseDialog bd, Field fi) {

    }

    private void enumField(Table t, Field fi, Object obj) {
        Log.info("enum " + R.getName(fi) + " " + R.getField(fi, obj));
        t.table(c -> {
            if (R.getField(fi, obj) != null) c.label(() -> "[orange]" + R.getType(fi).getSimpleName() + "[white] " + R.getName(fi) + ": " + R.getField(fi, obj).toString() + "     ");
            else c.label(() -> "[orange]" + R.getType(fi).getSimpleName() + "[white] " + R.getName(fi) + ": [yellow]null[white]" + "     ");
            c.button(Icon.settings, () -> {
                BaseDialog bd = new BaseDialog("Выбор");
                bd.cont.label(() -> "[orange]" + R.getType(fi).getSimpleName() + "[white]: ");
                bd.cont.row();
                bd.cont.table(k -> {
                    for (Object e : R.getType(fi).getEnumConstants()) {
                        k.button(e.toString(), () -> {
                            R.setField(fi, obj, e);
                        }).minSize(Core.graphics.getWidth()-20f, 30);
                        k.row();
                    }
                });
                bd.addCloseButton();
